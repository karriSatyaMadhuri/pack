...
def packaging_app():
    ...
    # Inline display
    if highway:
        st.write(f"➡ Highway Share: {route_pct.get('Highway', 0):.1f}%")
    if semiurban:
        st.write(f"➡ Semi-Urban Share: {route_pct.get('Semi-Urban', 0):.1f}%")
    if village:
        st.write(f"➡ Village Share: {route_pct.get('Village', 0):.1f}%")
